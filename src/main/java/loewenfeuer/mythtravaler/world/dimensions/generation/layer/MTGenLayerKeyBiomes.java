...
public enum MTGenLayerKeyBiomes implements IAreaTransformer1 {
	...
	private int getKeyBiomeFor(IExtendedNoiseRandom<?> random, int mapX, int mapZ, int index) {
		...
		switch ((index + offset) % 4) {
			case 0:
			default:
				return Registry.BIOME.getId(MTBiomes.deadDesHills.get());
			case 1:
				return Registry.BIOME.getId(MTBiomes.deadDes.get());
		}
	}
}